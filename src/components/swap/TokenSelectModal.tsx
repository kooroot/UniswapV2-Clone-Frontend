...
import { POPULAR_TOKENS, NATIVE_TOKEN } from '../../constants/tokens'
...
interface TokenSelectModalProps {
  isOpen: boolean
  onClose: () => void
  onSelect: (token: any) => void // Token | NATIVE_TOKEN
  otherToken: any  // Token | NATIVE_TOKEN
}
...
const TokenSelectModal = ({ isOpen, onClose, onSelect, otherToken }: TokenSelectModalProps) => {
  ...
  const filteredTokens = POPULAR_TOKENS.filter(token => {
    if (token.address === 'ETH') return true;
    return (
